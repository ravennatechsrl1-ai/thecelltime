...
export default function HomeCategoryNav() {
  ...
  return (
    <section className="border-y border-brand-gray-200 bg-[#f7f7f7] py-10 sm:py-12">
      <div className="container-app">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 lg:gap-4">
          {categories.map((cat) => (
            ...
            >
              <NavIconWrap>{cat.icon}</NavIconWrap>
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold uppercase tracking-widest text-brand-gray-400">
                  {cat.subtitle}
                </p>
                <p className="mt-0.5 text-sm font-bold uppercase tracking-wide text-brand-navy transition-colors group-hover:text-brand-black">
                  {cat.title}
                </p>
              ...
              <IconChevronRight className="h-4 w-4 shrink-0 text-brand-gray-300 transition-colors group-hover:text-brand-black" />
            </Link>
          ))}
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-brand-gray-200 pt-6">
          ...
          {brands.map((brand) => (
            <Link
              key={brand.href}
              href={brand.href}
              className="inline-flex min-h-[36px] items-center border border-brand-gray-200 bg-white px-4 text-xs font-semibold uppercase tracking-wide text-brand-gray-700 transition-colors hover:border-brand-black hover:text-brand-black"
            >
              {brand.label}
            </Link>
          ))}
