...
interface ProtectionMegaMenuProps {
  label: string;
  icon: ReactNode;
  inverted?: boolean;
}

export default function ProtectionMegaMenu({
  label,
  icon,
  inverted = false,
}: ProtectionMegaMenuProps) {
  const { t } = useLanguage();
  const [open, setOpen] = useState(false);
  const [deviceType, setDeviceType] = useState<ProtectionDeviceType | null>(null);
  ...
  const activeBrand: ProtectionNavBrand | null = useMemo(() => {
  ...
  function selectDevice(next: ProtectionDeviceType) {
    setDeviceType(next);
                        ...
                        {group.recent.length > 0 ? (
                          <div className="mb-2">
                            <p className="mb-1 text-[9px] font-bold uppercase tracking-widest text-brand-gray-400">
                              {t.protection.mostRecent}
                            </p>
                            <ul className="space-y-0.5">
                              {group.recent.map((model) => (
                                <li key={`r-${model.slug}`}>
                                  <Link
                                    href={protectionShopPath(
