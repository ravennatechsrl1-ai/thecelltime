...
import { RepairIssueId } from "@/lib/i18n";
...
export default function RepairPage() {
  ...
  const [selectedIssue, setSelectedIssue] = useState<RepairIssueId | "">("");
  ...
  const availableModels =
    DEVICE_OPTIONS.find((d) => d.brand === selectedBrand)?.models ?? [];
  ...
  function handleIssueSelect(issueId: RepairIssueId, basePrice: number) {
    setSelectedIssue(issueId);
    setEstimatedPrice(basePrice);
  }

  function canProceedStep1(): boolean {
    return selectedBrand.length > 0 && selectedModel.length > 0;
  }
  ...
  async function handleSubmit() {
    if (!canSubmitStep3() || !selectedIssue) return;
      ...
      <div className="mx-auto max-w-xl">
        {step === 1 && (
          <section className="border border-brand-gray-200 p-5 sm:p-8">
            ...
            <div className="mt-6 space-y-5">
              <div>
                <label htmlFor="brand" className="mb-2 block text-xs font-semibold uppercase tracking-wide text-brand-gray-500">
                  {t.repair.brand}
                </label>
                <select
                  id="brand"
              ...
              <div>
                <label htmlFor="model" className="mb-2 block text-xs font-semibold uppercase tracking-wide text-brand-gray-500">
                  {t.repair.model}
                </label>
                <select
                  id="model"
            ...
            <ul className="mt-6 space-y-3">
              {REPAIR_ISSUES.map((issue) => (
                <li key={issue.id}>
                  <button
                    type="button"
                    onClick={() => handleIssueSelect(issue.id, issue.basePrice)}
        ...
        {step === 3 && (
          <section className="border border-brand-gray-200 p-5 sm:p-8">
            ...
            <p className="mt-2 text-sm text-brand-gray-600">
              {t.repair.baseEstimate}: <strong>{formatPrice(estimatedPrice)}</strong>
            </p>
            ...
            >
              <div>
                <label htmlFor="name" className="mb-2 block text-xs font-semibold uppercase tracking-wide text-brand-gray-500">
                  {t.repair.name}
                </label>
                <input
                  id="name"
              ...
              <div>
                <label htmlFor="phone" className="mb-2 block text-xs font-semibold uppercase tracking-wide text-brand-gray-500">
                  {t.repair.phone}
                </label>
                <input
                  id="phone"
              ...
              <div>
                <label htmlFor="email" className="mb-2 block text-xs font-semibold uppercase tracking-wide text-brand-gray-500">
                  {t.repair.email}
                </label>
                <input
                  id="email"
